...
/** State: Initial state.*/
#define INITIAL 				0
/** State: Waiting for ACK associated to POST.*/
#define WAIT_ACK_POST 				1
/** State: Waiting EAP from EAP stack*/
#define WAIT_EAP_MSG 				2
/** State: Initial state.*/
#define WAIT_ACK_PUT				3
/** State: Initial state.*/
#define WAIT_ACK_AUTH_PUT		        4
/** State: Initial state.*/
#define AUTH		                        5
/** State: Initial state.*/
#define CLOSED		                        6
...
static char * state_name[] = {"NO CHANGE", "INITIAL", "WAIT_ACK_POST", "WAIT_EAP_MSG",
        "WAIT_ACK_PUT", "WAIT_ACK_AUTH_PUT",
        "AUTH","CLOSED"};
 ...
 * */
int _transition(coap_eap_ctx *coap_eap_session);
 ...
 * */
int _livenessTestPeer();
/** 
 * Response to a liveness test. A PNA is sent.
 * @return NO_CHANGE If the message was sent succesfully.
 * @return ERROR If an error ocurred during trasmission. 
 * */
int _livenessTestResponse();
/**
